...
class BodyWeightType(ObjectType):
    ...
    bodyWeight = Float(description="Animal body weight (kg)")
    weightGain = Float(description="Weight gain (kg)")
...
class BodyWeightQuery(ObjectType):
    bodyweight = List(
        BodyWeightType,
        ...
        limit=Argument(Int, description="Limit number of bodyweight records"),
        description="Query bodyweight data by animal, day, or weight values"
    )

    def resolve_bodyweight(self, info, **kwargs):
        query = {}
        for field in ["animalIdentifier", "day", "bodyWeight", "weightGain"]:
            if field in kwargs:
                query[field] = kwargs[field]
        for field, prefix in [("bodyWeight", "bodyWeight"), ("weightGain", "weightGain")]:
