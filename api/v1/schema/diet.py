...
class DietType(ObjectType):
    ...
    crudeProteinOfDryMatter = Float(description="Crude protein content of dry matter (%)")
    crudeFatContentOfDryMatter = Float(description="Crude fat content of dry matter (%)")
    neutralDetergentFiberContentOfDryMatter = Float(description="Neutral detergent fiber of dry matter (%)")
    organicMatterContentOfDryMatter = Float(description="Organic matter content of dry matter (%)")
...
class DietQuery(ObjectType):
    diet = List(
        ...
        limit=Argument(Int, description="Limit number of diet records"),
        description="Query diet data by animal, day, or nutrient values"
    )

    def resolve_diet(self, info, **kwargs):
        query = {}
        for field in ["animalIdentifier", "day", "crudeProteinOfDryMatter"]:
            if field in kwargs:
                query[field] = kwargs[field]
