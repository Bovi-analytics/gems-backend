...
# from schema.gas import GasType

class DigestibilityType(ObjectType):
    ...
    grossEnergyDigestibility = Float(description="Gross energy digestibility (%)")
    dryMatterDigestibility = Float(description="Dry matter digestibility (%)")
...
class DigestibilityQuery(ObjectType):
    digestibility = List(
        DigestibilityType,
        ...
        limit=Argument(Int, description="Limit number of digestibility records"),
        description="Query digestibility data by animal, day, or digestibility metrics"
    )

    def resolve_digestibility(self, info, **kwargs):
        query = {}
        for field in ["animalIdentifier", "day", "grossEnergyDigestibility", "dryMatterDigestibility"]:
            if field in kwargs:
                query[field] = kwargs[field]
        for field, prefix in [("grossEnergyDigestibility", "grossEnergyDigestibility"), ("dryMatterDigestibility", "dryMatterDigestibility")]:
